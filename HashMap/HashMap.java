...
public class HashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        // call put() with a flag giving it permission to resize
     ...
     * @param key key to add into the HashMap
     * @param value value to add into the HashMap
     * @param resizeFlag gives permission to call resize
     * @throws IllegalArgumentException if key or value is null
     * @return null if the key was not already in the map. If it was in the
     * map, return the old value associated with it
     */
    private V put(K key, V value, boolean resizeFlag) {
        ...
        if (loadFactor > MAX_LOAD_FACTOR && resizeFlag) {
        ...
        // instanciate node
        MapEntry<K, V> node = table[hashNum];

        // search linked list for <key, value> pair
        while (node != null) {
            // handle bucket with matching <key, value> pair
        ...
        // handle bucket without matching <key, value> pair
        MapEntry<K, V> newNode = new MapEntry(key, value, table[hashNum]);
    ...
    public void resizeBackingTable(int length) {
        ...
        if (length <= 0 || length < size) {
            throw new IllegalArgumentException(
            "Length is non-positive or less than size of bucket");
        }

        // save table data
        MapEntry<K, V>[] savedTable = table;

        // instanciate new table
        table = (MapEntry<K, V>[]) new MapEntry[length];

        // loop through buckets
        for (MapEntry<K, V> node: savedTable) {
            // loop though linked list
            while (node != null) {
                // call put() method with resizeFlag set to false
    ...
    public V remove(K key) {
        ...
        if (key == null) {
            throw new IllegalArgumentException(
            "Null keys not allowed in the data structure");
        }
    ...
    public Set<K> keySet() {
        ...
        for (MapEntry<K, V> node: table) {
            ...
            while (node != null) {
                ...
                // get next elemet on linked list
                node = node.getNext();
            }
        }
        return keys;
